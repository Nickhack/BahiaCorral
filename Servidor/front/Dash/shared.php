...
function cToFDirect($field){
	return round((float)(1.8*$field + 32), 1);
}

// Pressure
function mbToin(&$weather, $field){
	if(!isset($weather[$field])) return;
	$weather[$field] = round((float)(0.02953*$weather[$field]), 2);
}

function inTomb(&$weather, $field){
	if(!isset($weather[$field])) return;
	$weather[$field] = round((float)(33.86*$weather[$field]), 2);
}
...
function getUpdatedString($datetime) {
 ...
 if($Conn->connect_errno)
 {
  ...
  $query_info = "SELECT * FROM datos ORDER BY FECHA DESC limit 1";
  ...
  $hora= explode(" ", $ultima[8]);
	
	if ($showDate) {
		return '<div class="updatedtime"><span>' . $hora[0] . '</span><br />' . $hora[1] . '</div>';
	}
	else {
		return '<div class="updatedtime"><span>Actualizado <br />' . $hora[1] . '</div>';
	}
}
